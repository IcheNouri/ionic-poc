...
function accountWithRoles(roles: string[]): Account {
  return {
    activated: true,
    roles,
    operations: [],
  ...
  describe('Account Service', () => {
    ...
    beforeEach(() => {
      TestBed.configureTestingModule({
        ...
        providers: [TranslateService, StateStorageService, { provide: Router, useValue: router }],
      });
    ...
    describe('authenticate', () => {
      it('authenticationState should emit null if input is null', () => {
        // GIVEN
        let userIdentity: Account | null = accountWithRoles([]);
      ...
      it('authenticationState should emit the same account as was in input parameter', () => {
        // GIVEN
        const expectedResult = accountWithRoles([]);
    ...
    describe('hasAnyRole', () => {
      describe('hasAnyRole string parameter', () => {
        it('should return false if user is not logged', () => {
          const hasRole = service.hasAnyRole(Role.USER);
          expect(hasRole).toBe(false);
        });

        it('should return false if user is logged and has not role', () => {
          service.authenticate(accountWithRoles([Role.USER]));

          const hasRole = service.hasAnyRole(Role.ADMIN);

          expect(hasRole).toBe(false);
        });

        it('should return true if user is logged and has role', () => {
          service.authenticate(accountWithRoles([Role.USER]));
      ...
      describe('hasAnyRole array parameter', () => {
        it('should return false if user is not logged', () => {
          const hasRole = service.hasAnyRole([Role.USER]);
        ...
        it('should return true if user is logged and has role', () => {
          ...
          const hasRole = service.hasAnyRole([Role.USER, Role.ADMIN]);
